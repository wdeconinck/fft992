...
!     PSCALE IS THE SCALE FACTOR USED ON THE ISIGN=-1 (R2C) PATH
!           NORMALLY this is 1/N BUT MAY BE SET TO 1
!           IF NORMALIZATION IS TO BE PERFORMED BY ANOTHER ROUTINE
!
...
!
!     ISIGN=-1: A(K)=PSCALE*SUM(J=0,...,N-1)(X(J)*COS(2*J*K*PI/N))
!               B(K)=-PSCALE*SUM(J=0,...,N-1)(X(J)*SIN(2*J*K*PI/N))
!
#ifdef MATHKEISAN
! MathKeisan is a scientific library optimized for NEC (www.mathkeisan.com)

      SUBROUTINE FFT992(A,TRIGS_,IFAX_,INC,JUMP,N,LOT,ISIGN,PSCALE)
!AUTOPROMOTE
#ifdef FFT992_PRECISION
USE ISO_FORTRAN_ENV, ONLY : JPIM => INT32, JPRB => FFT992_PRECISION
#else
...
!     ISIGN = +1 FOR TRANSFORM FROM SPECTRAL TO GRIDPOINT
!           = -1 FOR TRANSFORM FROM GRIDPOINT TO SPECTRAL
!     PSCALE IS THE SCALE FACTOR USED ON THE ISIGN=-1 PATH
!
!     ORDERING OF COEFFICIENTS:
!         A(0),B(0),A(1),B(1),A(2),B(2),...,A(N/2),B(N/2)
!         WHERE B(0)=B(N/2)=0; (N+2) LOCATIONS REQUIRED
...
!
      SUBROUTINE FFT992(A,TRIGS,IFAX,INC,JUMP,N,LOT,ISIGN,PSCALE)
...
!      
      ...
      INTEGER(KIND=JPIM), PARAMETER :: IFAX_MAX_INDEX = 10 ! matches size of IFAX in set99b
...
#include "abor1.intfb.h"
      ...
      IF (LHOOK) CALL DR_HOOK('FFT992',0,ZHOOK_HANDLE)
      NFAX=IFAX(1)
      IF (NFAX.GT.IFAX_MAX_INDEX) THEN
        CALL ABOR1('FFT992: NUMBER OF FACTORS EXCEEDS MAXIMUM ALLOWED')
      ENDIF
      IF (ISIGN.EQ.+1) THEN
!
...
!
     ...
     &        K.GT.(1+MOD(NFAX,2)).AND.K.LT.NFAX) THEN
          ...
          IF (IGO.EQ.+1) THEN
!DEC$ FORCEINLINE
           CALL QPASSF(A(IA),A(IA+IFAC*LA*INCA),WORK(1),WORK(LA*LOT+1),PSCALE, &
     &                 TRIGS,INCA,LOT,JUMPA,1,LOT,N,IFAC,LA,IERR,LIPL)
          ELSE
!DEC$ FORCEINLINE
           CALL QPASSF(WORK(1),WORK(IFAC*LA*LOT+1),A(IA),A(IA+LA*INCA),PSCALE,&
     &                 TRIGS,LOT,INCA,1,JUMPA,LOT,N,IFAC,LA,IERR,LIPL)
          ENDIF
          IF (IERR.NE.0) THEN
            IF (IERR.EQ.2) WRITE(6,901) IFAC
...
!     Z IS THE NORMALIZATION FACTOR FOR THE TRANSFORM WHEN LA=N/IFAC (DIRTRANS)
!     TRIGS IS A PRECALCULATED LIST OF SINES & COSINES
!     INC1 IS THE ADDRESSING INCREMENT FOR A
!     INC2 IS THE ADDRESSING INCREMENT FOR C
!     INC3 IS THE INCREMENT BETWEEN INPUT VECTORS A
...
!-----------------------------------------------------------------------
!
      SUBROUTINE QPASSF(A,B,C,D,Z,TRIGS,INC1,INC2,INC3,INC4,LOT,N,IFAC, &
     &    LA,IERR,LIPL)
!AUTOPROMOTE
#ifdef FFT992_PRECISION
USE ISO_FORTRAN_ENV, ONLY : JPIM => INT32, JPRB => FFT992_PRECISION
