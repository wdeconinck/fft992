...
#include "abor1.intfb.h"
      ...
      INTEGER(KIND=JPIM), PARAMETER :: IFAX_MAX_INDEX = 10
      INTEGER(KIND=JPIM) :: I,IFAC,IL,IXXX,K,NHL,NIL,NFAX,NU
      REAL(KIND=JPRB) :: ANGLE,DEL
      INTEGER(KIND=JPIM) :: JFAX(IFAX_MAX_INDEX),NLFAX(7)
!
...
!
      IF (SIZE(IFAX) < IFAX_MAX_INDEX) THEN
        BLOCK
          CHARACTER(LEN=200) :: msg
          WRITE(msg, '(A,I0,A)') 'SET99B: SIZE OF IFAX ARRAY IS TOO SMALL. IT NEEDS TO BE AT LEAST ', IFAX_MAX_INDEX, '.'
          CALL ABOR1(msg)
        END BLOCK
      ENDIF
!
