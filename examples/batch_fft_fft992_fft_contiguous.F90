program batch_fft_fft992_fft_contiguous
...
integer(kind=int32) :: inc, jump, point, field
logical :: lusefft992
...
! FFT-contiguous layout: WORK(point, field)
inc = 1_int32
jump = nfreq * 2
...
contains
  ...
  subroutine print_spectrum(values)
    ...
    write(*,'(a)') 'batch_fft_fft992_fft_contiguous spectra:'
    do field = 1, batch
  ...
  subroutine validate_spectrum_harmonic(harmonic, cos_amplitude, sin_amplitude)
    ...
    if (max_error > tolerance) then
      write(*,'(a,1x,es12.4)') 'batch_fft_fft992_fft_contiguous spectrum validation failed; max error =', max_error
      stop 1
    end if

    write(*,'(a,1x,es12.4)') 'batch_fft_fft992_fft_contiguous spectrum validation passed; max error =', max_error
  ...
  subroutine validate_roundtrip
    ...
    if (max_error > tolerance) then
      write(*,'(a,1x,es12.4)') 'batch_fft_fft992_fft_contiguous validation failed; max error =', max_error
      stop 1
    end if

    write(*,'(a,1x,es12.4)') 'batch_fft_fft992_fft_contiguous validation passed; max error =', max_error
  end subroutine validate_roundtrip

end program batch_fft_fft992_fft_contiguous