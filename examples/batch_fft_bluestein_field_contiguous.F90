program batch_fft_bluestein_field_contiguous
...
! Default layout is batch-contiguous; no explicit layout argument provided.
call bluestein_fft(tb, n, direction_r2c, batch, work)

work(:,:) = work(:,:) * scale_r2c
call print_spectrum(work)
call validate_spectrum_harmonic(harmonic, cos_amplitude, sin_amplitude)
call bluestein_fft(tb, n, direction_c2r, batch, work)
...
contains
  ...
  subroutine print_spectrum(values)
    ...
    write(*,'(a)') 'batch_fft_bluestein_field_contiguous spectra:'
    do field = 1, batch
  ...
  subroutine validate_spectrum_harmonic(harmonic, cos_amplitude, sin_amplitude)
    ...
    if (max_error > tolerance) then
      write(*,'(a,1x,es12.4)') 'batch_fft_bluestein_field_contiguous spectrum validation failed; max error =', max_error
      stop 1
    end if

    write(*,'(a,1x,es12.4)') 'batch_fft_bluestein_field_contiguous spectrum validation passed; max error =', max_error
  ...
  subroutine validate_roundtrip
    ...
    if (max_error > tolerance) then
      write(*,'(a,1x,es12.4)') 'batch_fft_bluestein_field_contiguous validation failed; max error =', max_error
      stop 1
    end if

    write(*,'(a,1x,es12.4)') 'batch_fft_bluestein_field_contiguous validation passed; max error =', max_error
  end subroutine validate_roundtrip

end program batch_fft_bluestein_field_contiguous